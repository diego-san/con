...
def in_datos_productos(precio, fuente, fechas, fechapub, id_cambio_id, id_reg_id, id_sub, nombre):
    img = iconos(id_sub)
    conn = sqlite3.connect(dir+'/../db.sqlite3')
    c = conn.cursor()
    c.execute('''INSERT INTO  basedatos_bienesyservicios VALUES(?,?,?,?,?,?,?,?,?,?,?,?)''', (None, str(nombre), str(precio), img, str(fuente), str(fechas), str(fechapub), str(id_cambio_id), str(id_reg_id), str(id_sub), 'unidad', 1))
    conn.commit()
    conn.close()
